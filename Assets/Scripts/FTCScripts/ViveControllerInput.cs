...
namespace FTC {
	public class ViveControllerInput : BaseInputModule, IViveControllerInput
	{
	    ...
	    protected override void Start()
	    {
	        ...
	        if (Initialized == false)
	        {
				...
				ControllerCamera.depth = -2f;
				ControllerCamera.nearClipPlane = 0.001f;
				ControllerCamera.farClipPlane = 5f;
	    ...
	    private bool GetLookPointerEventData(int index)
	    {
			...
			PointEvents[index].position = ControllerCamera.WorldToScreenPoint(ControllerCamera.transform.position + ControllerCamera.transform.forward);
	    ...
	    private void UpdateCameraPosition(int index)
	    {
			...
			Debug.DrawLine (ControllerCamera.transform.position, ControllerCamera.transform.position + (ControllerCamera.transform.forward * 10));
	    }
	    ...
	    {
			return (ControllerDevices[index] != null && ControllerDevices[index].GetPressDown(Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad) == true);
	    }

	    private bool ButtonUp(int index)
	    {
			return (ControllerDevices[index] != null && ControllerDevices[index].GetPressUp(Valve.VR.EVRButtonId.k_EButton_SteamVR_Touchpad) == true);
	    }
	}
}